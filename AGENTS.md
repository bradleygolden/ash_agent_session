...
- Keep unit tests in `test/ash_agent_session`, mirroring `lib/` structure with `<filename>_test.exs`, and default to `async: true` when isolation is possible.
...
```
test/
├── ash_agent_session/           # Unit tests (mirror lib/ structure)
...
defmodule AshAgentSession.Integration.Metadata.ReqLLM.OpenAITest do
  use AshAgentSession.IntegrationCase, backend: :req_llm, provider: :openai
